...
struct ScopeInner {
    ...
    cursor: Cell<usize>,
    dirty: Rc<Cell<bool>>,
    context: RefCell<HashMap<TypeId, Box<dyn Any>>>,
}
...
/// scope nested inside it via [`Scope::render`] — not to the scope's own
/// future renders, which must provide it again.
///
/// # Panics
///
/// Panics if called outside a [`Scope::render`] pass.
pub fn provide_context<T: 'static>(value: T) {
    ACTIVE_SCOPES.with(|scopes| {
        ...
        let scope = scopes.last().unwrap_or_else(|| {
            panic!(
                "provide_context called outside of Scope::render — hooks must run \
                 during a component tree's render pass"
            )
        });
        scope
...
pub fn use_context<T: Clone + 'static>() -> Option<T> {
    ACTIVE_SCOPES.with(|scopes| {
        ...
        if scopes.is_empty() {
            panic!(
                "use_context called outside of Scope::render — hooks must run \
                 during a component tree's render pass"
            );
        }
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "use_context called outside of Scope::render")]
    fn use_context_outside_a_render_panics() {
        use_context::<i32>();
    }

    #[test]
    #[should_panic(expected = "provide_context called outside of Scope::render")]
